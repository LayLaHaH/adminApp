...
class MarketSettings extends StatefulWidget {
  const MarketSettings({super.key});

  @override
  State<MarketSettings> createState() => _MarketSettingsState();
}
class _MarketSettingsState extends State<MarketSettings> {
 ...
 Future<List<Market>> getRequest() async {
    ...
    for (var gov in responseData) {
      try{
      Market market = Market.fromJson(gov);
      markets.add(market);
  ...
  void deleteAlert(int? id) async {
    ...
    if (results != null && results == true) {
      final response = await http.delete(Uri.parse('$baseUrl/Market/delete?id=$id'));
          ...
          child: Column(
            children:  <Widget>[
              ...
              Padding(
                ...
                child: LeftSideAddress(title:'Market Settings', fontSize: 20.0,),
              ),
              //add button
              myAddButton(label: "Add Market", route: "/addMarket", maxwidth: 130.0,),
              //search
              TextField(
                                    ...
                                    Expanded(flex: 2,child:Row(
                                      children: [
                                        IconButton( icon: Icon(Icons.edit,color: Color.fromARGB(255, 206, 134, 34),),
                                          onPressed: (){Navigator.pushNamed(context, '/editMarket',arguments: snapshot.data![index]);},),
